...
class SettingsController extends AppController {
	...
	date_default_timezone_set("America/Sao_Paulo");
        if ($this->request->is('post')) {
			...
			$this->savePreferences($this->request->data);
	...
	public function savePreferences($post){
	
		$abertura = $this->request->data['Company']['open_hour'];
		$fechamento = $this->request->data['Company']['close_hour'];
		$id = $this->Session->read('CompanyLoggedIn.Company.id');
		
		$workDaysArray = $this->request->data['Company']['workDays'];
		
		$workDays = '';
		
		if(!empty($workDaysArray['dom'])){
		if($workDaysArray['dom'] == 'on'){
			$workDays = $workDays.'dom,';
		}
		}
		
		if(!empty($workDaysArray['seg'])){
		if($workDaysArray['seg'] == 'on'){
			$workDays = $workDays.'seg,';
		}
		}
		
		if(!empty($workDaysArray['ter'])){
		if($workDaysArray['ter'] == 'on'){
			$workDays = $workDays.'ter,';
		}
		}
		
		if(!empty($workDaysArray['qua'])){
		if($workDaysArray['qua'] == 'on'){
			$workDays = $workDays.'qua,';
		}}
		
		if(!empty($workDaysArray['qui'])){
		if($workDaysArray['qui'] == 'on'){
			$workDays = $workDays.'qui,';
		}}
		
		if(!empty($workDaysArray['sex'])){
		if($workDaysArray['sex'] == 'on'){
			$workDays = $workDays.'sex,';
		}}
		
		if(!empty($workDaysArray['sab'])){
		if($workDaysArray['sab'] == 'on'){
			$workDays = $workDays.'sab,';
		}	}	
		
		$workDays = substr_replace($workDays, "", -1);
		
		$sql = "UPDATE companies SET `open_hour` = '{$abertura}', `close_hour` = '{$fechamento}', `work_days` = '{$workDays}' WHERE id =  {$id};";
