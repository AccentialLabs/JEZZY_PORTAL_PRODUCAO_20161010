...
class DashboardController extends AppController {
	...
	$isSelectingPlan = $this->Session->read("isSelectingPlan");
		if(isset($isSelectingPlan)){
				 
				 if($isSelectingPlan == true){
		...
		$this->set('comissionsMonth', $this->getAllComissionamentMonth());
		$this->set('checksToday', $this->getAllCheckoutsToday());
		$this->set('checksMonth', $this->getAllCheckoutsMonth());
		...
		public function isNotFirst(){
			$this->autoRender = false;
			 $this->Session->write('dashboardFirstLogin', 0);
	...
	public function reproveScheduleSolicitation() {
		...
		$this->sendMobileNotification($solicitation[0]['schedules_solicitation']['user_id'], "Ops... Este horário está indisponível. Escolha um outro."); 
    }
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        <table border="0" cellpadding="0" cellspacing="0"  >
            ...
            <tr>
                <td colspan="4" style="background-image: url(\'https://secure.jezzy.com.br/uploads/Emails/files/aniversariante/aniver_1.jpg\'); height: 202px; color: #dab86e; font-family: Tahoma; font-size: 5em; padding-left: 55px;">'.$userName.',</td>
            </tr>
            <tr>
                <td colspan="4"><img src="https://secure.jezzy.com.br/uploads/Emails/files/aniversariante/aniver_2.jpg"  style="vertical-align: bottom;"/></td>
            </tr>
            <tr>
                <td colspan="4"><img src="https://secure.jezzy.com.br/uploads/Emails/files/aniversariante/aniver_3.jpg"  style="vertical-align: bottom;"/></td>
            </tr>

            <tr>
                <td colspan="4"><img src="https://secure.jezzy.com.br/uploads/Emails/files/aniversariante/aniver_4.jpg"  style="vertical-align: bottom;"/></td>
            </tr>
            <tr>
                   <td colspan="4" style="background-image: url(\'https://secure.jezzy.com.br/uploads/Emails/files/aniversariante/aniver_5.jpg\'); height: 108px; color: #fff; font-family: Tahoma; font-size: 1.6em; padding-left: 55px; vertical-align: top;"><i>'.$compFancyName.'</i></td>
            </tr>
...
//}';
       ...
       // $header [] = "Authorization: OAuth fe4thhxju5ll725ftnquf1o716y6kpu";
        //$auth = 'fe4thhxju5ll725ftnquf1o716y6kpu';
		  ...
		  $header [] = "Authorization: OAuth 7724f1c0479a4553b3ec65fd7888b076_v2";
        $auth = '7724f1c0479a4553b3ec65fd7888b076_v2';
		...
		$sqlMoIPAccount = "UPDATE companies SET moip_id = '{$dadosMoip['additionalInfo']['account']['id']}', moip_account = '{$dadosMoip['additionalInfo']['account']['login']}' WHERE id = {$company['Company']['id']};";
			$this->Session->write('CompanyLoggedIn.Company.moip_id', $dadosMoip['additionalInfo']['account']['id']);
       ...
       $sqlMoIPAccount = "UPDATE companies SET moip_id = '{$dadosMoip['id']}', moip_account = '{$dadosMoip['login']}', link_setpassword_moip = '{$dadosMoip['_links']['setPassword']['href']}' WHERE id = {$company['Company']['id']};";
		$this->Session->write('CompanyLoggedIn.Company.moip_id', $dadosMoip['id']);
	}
...
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
        ...
        $emailBody = '  <table border="0" cellpadding="0" cellspacing="0"  style="background: #f2f2f2;">
            ...
            <tr>
               ...
               <tr>
                <td colspan="1"><img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/07.jpg" width="151" style="vertical-align: bottom; text-align: center;"/></td>
                <td  colspan="1"><img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/08.jpg" width="151" style="vertical-align: bottom; text-align: center;"/></td>
                <td colspan="1"> <img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/09.jpg" width="151" style="vertical-align: bottom; text-align: center;"/></td>
                <td colspan="1"><img src="https://secure.jezzy.com.br/uploads/Emails/files/transacao-finalizada/10.jpg" width="151" style="vertical-align: bottom; text-align: center;"/></td>
            </tr>
             <tr>
...
inner join offers on offers.id = checkouts.offer_id
 where services_vouchers.id = {$id};";
	...
	*/
	...
	public function getSchedulesBySecondUser(){
	
		$this->layout = "";
	 $company = $this->Session->read('CompanyLoggedIn');
	 $secondUser = $this->request->data['secondID'];
	 ...
	 if(strpos($date, '/') !== false){
		...
		$realDate = $dateArray[2].'-'.$dateArray[1].'-'.$dateArray[0];
		...
		$sql = " SELECT schedules.*, secondary_users.*
            FROM schedules
            INNER JOIN secondary_users
                ON schedules.secondary_user_id = secondary_users.id
            WHERE schedules.companie_id = {$company['Company']['id']}
            AND schedules.date  = '{$realDate}'
			AND schedules.secondary_user_id = {$secondUser}
			order by date ASC, time_begin ASC;";
	...
	public function getAllCheckoutsToday(){
		//$this->autoRender = false;
	
		$company = $this->Session->read('CompanyLoggedIn');
		$sql = "select * from checkouts WHERE DATE(date) = CURDATE() and payment_state_id = 4 and company_id =  {$company['Company']['id']};";
       ...
       $checkouts = $this->AccentialApi->urlRequestToGetData('users', 'query', $param);
	   ...
	   if(!empty($checkouts)){
				...
				foreach($checkouts as $check){
					...
					$total = $total + $check['checkouts']['total_value'];
	...
	public function getAllCheckoutsMonth(){
	
	//$this->autoRender = false;
	
		$company = $this->Session->read('CompanyLoggedIn');
		$month = date('m');
		$sql = "select * from checkouts WHERE  MONTH(date) = {$month}  and payment_state_id = 4 and company_id =  {$company['Company']['id']};";
	...
	public function getAllComissionamentMonth(){
	
		$company = $this->Session->read('CompanyLoggedIn');
		$month = date('m');
		$sql = "select count(*) quantity, SUM(financial_parameters_results.vl_salao) val_total from financial_parameters_results 
		inner join checkouts on checkouts.id = financial_parameters_results.checkout_id
		where financial_parameters_results.company_id = {$company['Company']['id']} and MONTH(financial_parameters_results.date_register) = {$month} and checkouts.payment_state_id = 4;";
