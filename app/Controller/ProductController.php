			...
			class ProductController extends AppController {
				...
				public function productManipulation($offerId = null) {
					...
					if ($offerId != null && isset($offerId)) {
						...
						$this->set('offerService', $this->getServiceByEditedOffer($offerId));
...
public function saveFilters() {
        ...
        if ($this->request->is('post')) {
            ...
            if (isset($this->request['data']['gender']) && !empty($this->request['data']['gender'])) {
                if(is_array($this->request['data']['gender'])){
                   $gender =  implode(",", $this->request['data']['gender']);
                }else{
            ...
            if (isset($this->request['data']['age'])&& !empty($this->request['data']['age'])) {
                if(is_array($this->request['data']['age'])){
                     $age = implode(",", $this->request['data']['age']);
                }else{
            ...
            if (isset($this->request['data']['hair_type'])&& !empty($this->request['data']['hair_type'])) {
                  if(is_array($this->request['data']['hair_type'])){
                      $hair_type = implode(",", $this->request['data']['hair_type']);
                  }else{
                      $hair_type = $this->request['data']['hair_type'];
                  }
            ...
            if ($offers_filters_id == "") {
                $query = "
                 ...
                 $query2 = "
                    ...
                    (offer_id, hair_type, public)
                    VALUES
                    ('" . $offerId . "','" . $hair_type . "','" . $gender . "');";
            ...
            $abc = $this->AccentialApi->urlRequestToGetData('users', 'query', $paramsQuery);
            $abc2 = $this->AccentialApi->urlRequestToGetData('users', 'query', $paramsQuery2);
			...
			*/
			$sqlSelectFilter = "select * from offers_filters where offer_id = {$offerId};";
                        ...
                        $sqlSelectFilter2 = "select * from offers_questions where offer_id = {$offerId};";
                        
			$paramsMyOffersFilters = array('User' => array('query' => $sqlSelectFilter));
                        $paramsMyOffersFilters2 = array('User' => array('query' => $sqlSelectFilter2));
                        
			$myOffersFilters = $this->AccentialApi->urlRequestToGetData('users', 'query', $paramsMyOffersFilters);
                        $myOffersFilters2 = $this->AccentialApi->urlRequestToGetData('users', 'query', $paramsMyOffersFilters2);    
			if(!empty($myOffersFilters)){
				...
				$myOffersFilters[0]['offers_filters']['religion'] == ''){
					...
					$this->AccentialApi->urlRequestToGetData('users', 'query', $sqlUpdateParams);
                        	...
                        	if(!empty($myOffersFilters2)){

				if($myOffersFilters2[0]['offers_questions']['hair_type'] == '' &&
				$myOffersFilters2[0]['offers_questions']['application'] == '' &&
				$myOffersFilters2[0]['offers_questions']['public'] == ''){
					$sqlUpdateOfferToPublic2= "update offers set public = 'ACTIVE' where id = {$offerId};";
					$sqlUpdateParams2 = array('User' => array('query' => $sqlUpdateOfferToPublic2));
					$this->AccentialApi->urlRequestToGetData('users', 'query', $sqlUpdateParams2);
				}
 ...
 public function addCharacteristicsandCategories(){
				...
				private function getAllFilters() {
                        ...
                        $return['public'] = $this->getFiltesOfProfiles('public', $company);
                           return $return;
                    }
				...
				private function getFiltesOfProfiles($filter, $company) {
                        $paramExtras = array(
                            'User' => array(
                                'query' => 'SELECT COUNT(facebook_profiles.' . $filter . ') as sum, facebook_profiles.' . $filter . ' as response FROM facebook_profiles INNER JOIN companies_users ON facebook_profiles.user_id = companies_users.user_id INNER JOIN companies_users response3 ON companies_users.user_id = facebook_profiles.user_id WHERE facebook_profiles.' . $filter . ' IS NOT NULL and companies_users.company_id = '.$company['Company']['id'].' and companies_users.status != "INACTIVE" GROUP BY ' . $filter . ';'
                            )
                        );
                        //print_r($paramExtras);
                    ...
                    private function getFiltesOfProfilesHairTypeandChemistry($filter, $filter2, $company) {
                            $paramExtras = array(
                                'User' => array(
                                    'query' => 'SELECT facebook_profiles.' . $filter . ' as response, facebook_profiles.' . $filter2 . ' as response2 FROM facebook_profiles INNER JOIN companies_users ON facebook_profiles.user_id = companies_users.user_id INNER JOIN companies_users as response3 ON companies_users.user_id = facebook_profiles.user_id WHERE facebook_profiles.' . $filter . ' IS NOT NULL and facebook_profiles.' . $filter . ' != " " and facebook_profiles.' . $filter2 . ' IS NOT NULL and facebook_profiles.' . $filter2 . ' != " " and companies_users.company_id = '.$company['Company']['id'].' and companies_users.status != "INACTIVE";'
                                )
                            );
							//print_r($paramExtras);
                            $result = $this->AccentialApi->urlRequestToGetData('users', 'query', $paramExtras);
       ...
       //print_r($query);
        ...
        return $offerCategories;
				...
				*/
				...
				public function getServiceByEditedOffer($offerId){

					
					$sql = "select * from offers_services inner join services on services.id = offers_services.service_id inner join subclasses on subclasses.id = services.subclasse_id  where offers_services.offer_id = ".$offerId.";";
					//print_r($sql);
					$params = array(
							'General' => array(
							'query' => $sql
					...
					$offerService = $this->mb_unserialize($this->AccentialApi->urlRequestToGetData('General', 'query', $params));
					return $offerService;
				}
