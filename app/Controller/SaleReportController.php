		...
		class SaleReportController extends AppController {
			...
			public function index() {
				...
				$this->set('allGeneralCheckouts', $this->getAllGerenalSales());
			}
			...
			/*$company = $this->Session->read('CompanyLoggedIn');
				...
				$todasWithComment = $this->getGerenalSalesByMonth($month);

				 $this->set('allSales', $todasWithComment); 
			}
			
			
			public function getAllSalesByMonthComplete(){
			$this->layout = "";	
			...
			/*$company = $this->Session->read('CompanyLoggedIn');
				...
				$todasWithComment = $this->getGerenalSalesByMonthCompleted($month);

				 $this->set('allSalesDone', $todasWithComment); 
			}
			
			public function getAllSalesByMonthPending(){
			$this->layout = "";	
			...
			/*$company = $this->Session->read('CompanyLoggedIn');
				...
				$todasWithComment = $this->getGerenalSalesByMonthPending($month);

				 $this->set('allSalesPending', $todasWithComment); 
			}
			
			
				public function getAllSalesByMonthPendentes(){
			$this->layout = "";	
			...
			/**
				RETORNA AS  SEGUINTES VENDAS:
					VENDAS DE PRODUTOS PRÓPRIOS
					VENDAS DE OFERTAS DE SERVIÇOS 
					VEDAS COMISSIONADAS JEZZY
			*/
			public function getAllGerenalSales(){
			
			$company = $this->Session->read('CompanyLoggedIn');
			
					$query = "select * from checkouts inner join offers on offers.id = checkouts.offer_id 
...
left join offers_comments on offers_comments.offer_id = offers.id
									where commissioned_company_id = {$company['Company'] ['id']} or checkouts.company_id = {$company['Company'] ['id']} group by checkouts.id order by checkouts.id;";
			...
			public function getGerenalSalesByMonth($month){
...
left join financial_parameters_results on financial_parameters_results.checkout_id = checkouts.id
left join offers_comments on offers_comments.offer_id = offers.id
 where   commissioned_company_id = {$company['Company'] ['id']} and MONTH(checkouts.date) = {$month} or checkouts.company_id = {$company['Company'] ['id']} and MONTH(checkouts.date) = {$month} 
group by checkouts.id order by checkouts.id;";
			...
			public function getGerenalSalesByMonthCompleted($month){
...
left join financial_parameters_results on financial_parameters_results.checkout_id = checkouts.id
left join offers_comments on offers_comments.offer_id = offers.id
 where  checkouts.payment_state_id = 4 and commissioned_company_id = {$company['Company'] ['id']} and MONTH(checkouts.date) = {$month} or checkouts.company_id = {$company['Company'] ['id']} and MONTH(checkouts.date) = {$month} 
group by checkouts.id order by checkouts.id;";
			...
			public function getGerenalSalesByMonthPending($month){
