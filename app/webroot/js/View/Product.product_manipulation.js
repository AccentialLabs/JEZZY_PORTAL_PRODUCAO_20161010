...
$(document).ready(function() {
	...
	$("#offers_questionsSaveButton").click(function(){
		...
		if(publics == ''){
			...
			$.ajax({
				url:getControllerPath("Product") +'addCharacteristicsandCategories',
				type: "POST",
				data: data
    ...
    $("#saveProduct").click(function() {
        ...
        if (validadeMinimalFilds()) {
			...
			var endDate = "";
			  if ($("#offerNoEnds").is(":checked")) {

					endDate = "02/12/2999";

			  }else{
				endDate = $("#dateHtmlEnd").val();
			  }
    ...
    $("#myModalOfferQuestions").on("submit", "#offerFormOptions", function() {
    ...
    $("#myModalOfferTarget").on("submit", "#offerTargetOptions", function() {
        ...
        var hair_type = $("input[name='hair_type[]']:checked").val();
        ...
        $('<input>').attr({type: 'hidden', value: gender, name: 'gender'}).appendTo('#offerTargetOptions');
        $('<input>').attr({type: 'hidden', value: hair_type, name: 'hair_type'}).appendTo('#offerTargetOptions');
        $('<input>').attr({type: 'hidden', value: age, name: 'age'}).appendTo('#offerTargetOptions');
        return true;
    });
